...
async function sendEmails() {
    ...
    try {
        const response = await sheets.spreadsheets.values.get({
            ...
            range: "Sheet1!A1:A" // Correct range with sheet name and column
        });

        const rows = response.data.values;
        if (!rows || rows.length <= 1) {
            return console.log("No subscribers found.");
        }

        const subscribers = rows.slice(1).map(row => ({ email: row[0] })); // Corrected to row[0]
